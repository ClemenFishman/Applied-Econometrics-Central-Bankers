function matrix2latex3(matrix, filename, varargin)

% Modified: T. Kam, Feb 10 2006.
...
% Enjoy life!!!
    ...
    if isnumeric(matrix)
        ...
        for h=1:height
            for w=1:width
                if(~isempty(format))
                    if abs(matrix{h, w})<0.01
                        format1 = '%-6.2d';
                    else
                        format1 = format;
                    end
                    matrix{h, w} = num2str(matrix{h, w}, format1);
                else
...
fprintf(fid, '%s \\\\\\hline \hline \r\n', colLabels{width});
    ...
    for h=1:height
        if(~isempty(rowLabels))
           ...
           fprintf(fid, '\\textbf{$ %s $}&', rowLabels{h});  % TK: row labels are math fonts
        end
%         for w=1:width-1
%              $\\
% \ $%
...
% [c]{c}%
% \ 0.83\\
% \left(  0.78,0.87\right)
% \end{array}
          fprintf(fid, '%s&');
          fprintf(fid, '\\begin{array}');
           fprintf(fid, '[c]{c}%\');
            fprintf(fid,  matrix{h, 1});
            fprintf(fid, '\\\left(');
             fprintf(fid,  matrix{h, 2});
              fprintf(fid, ',');
               fprintf(fid,  matrix{h, 3});
               fprintf(fid, '\right)');
               fprintf(fid, '\\end{array})');
%         end
                
        if h == height % Added by TK
            fprintf(fid, '%s\\\\\\hline\r\n', matrix{h, width});    
        else
            fprintf(fid, '%s\\\\\\\r\n', matrix{h, width});
        end
