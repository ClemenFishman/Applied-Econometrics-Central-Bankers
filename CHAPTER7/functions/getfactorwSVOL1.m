function [out,beta_tt]=...
    getfactorwSVOL1(data,H,F,Mux,beta0,P00,elast,hlastc, hlastw,hlaste,iamatc,iamatw,...
...
for jj=1:NC
    Q(j:j+nfact-1,j:j+nfact-1)=squeeze(iamatc(jj,:,:))*...
        diag(squeeze(hlastc(jj,i,1)).*Sbigc(jj,:))*squeeze(iamatc(jj,:,:))';
    j=j+nfact;
end
...
for jj=1:NC
    Q(j:j+nfact-1,j:j+nfact-1)=squeeze(iamatc(jj,:,:))*...
        diag(squeeze(hlastc(jj,i+1,1)).*Sbigc(jj,:))*squeeze(iamatc(jj,:,:))';
    j=j+nfact;
end
