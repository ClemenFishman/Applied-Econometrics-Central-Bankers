function [outh,outf,outg,outA]=getinitialvolcc(data0,REPS,BURN,T0,L)
muin=0.1;
N=cols(data0);
data=data0;
...
MU0=ones(N,1).*log(muin);%log(diag(s0)); %prior mean
...
g0=1;  %prior scale parameter for inverse gamma
Tg0=10;     %prior degrees of freedom
