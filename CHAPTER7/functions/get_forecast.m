function [ yy,yy1,hh,hh1] = get_forecast(N, HORZ,...
    T, L,LH, Y0,hlast0, ...
    beta1, Sbig1,iamat1,beta2,Sbig2,...
    iamat2,tar,tvar,delay,muvol,fvol,qvol,reps,scale,nml,EX)
...
for fi=L+1:HORZ+L
    ...
    %simulate data
     ...
     hhat(fi,:)=muvol+hhat(fi-1,:)*fvol+randn(1,1)*Cq;
        ...
        %
        hhat1(fi,:)=muvol+hhat1(fi-1,:)*fvol+randn(1,1)*0;
        xhat=[];
    xhat1=[];
