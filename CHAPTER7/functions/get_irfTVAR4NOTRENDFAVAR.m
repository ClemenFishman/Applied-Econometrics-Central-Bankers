function [ ir1,ir2,ir3,ir4] = get_irfTVAR4NOTRENDFAVAR(N, HORZ, T, L,LH, Y0,hlast0, beta1, Sbig1,iamat1,beta2,Sbig2,iamat2,tar,tvar,delay,muvol,fvol,qvol,reps,scale,nml,EX,fload,rmat)


%make H matrix
nfact=cols(fload);
NN=rows(fload);
H=zeros(NN+1,(nfact+1));
H(NN+1,nfact+1)=1;
H(1:NN,1:nfact)=fload;
...
zhat=zeros(HORZ+L,NN+1);
zhat1=zhat;
zhat2=zhat;
zhat3=zhat;
zhat4=zhat;
epsobs=randn(HORZ+L,NN+1)*RR;
epsobs1=randn(HORZ+L,NN+1)*RR;
epsobs2=randn(HORZ+L,NN+1)*RR;
epsobs3=randn(HORZ+L,NN+1)*RR;
epsobs4=randn(HORZ+L,NN+1)*RR;
ystar4=ystar;
ystar1=ystar;
ystar2=ystar;
ystar3=ystar;
for fi=L+1:HORZ+L
  ...
  %y2
    ...
    yhat4(fi,:) = ((xhat4*bb1 + uu1*csigma1)*e1)+...
        ((xhat4*bb2 + uu2*csigma2)*e2);
    zhat4(fi,:)=yhat4(fi,:)*H'+epsobs4(fi,:);
    %
    uu1=randn(1,N);uu2=randn(1,N);
    e1=ystar1(fi,:)<=tar;
    e2=ystar1(fi,:)>tar;
    yhat1(fi,:) = ((xhat1*bb1 + uu1*csigma1)*e1)+...
        ((xhat1*bb2 + uu2*csigma2)*e2);
    zhat1(fi,:)=yhat1(fi,:)*H'+epsobs1(fi,:);
    %
    uu1=randn(1,N);uu2=randn(1,N);
    e1=ystar2(fi,:)<=tar;
    e2=ystar2(fi,:)>tar;
    yhat2(fi,:) = ((xhat2*bb1 + uu1*csigma1)*e1)+...
        ((xhat2*bb2 + uu2*csigma2)*e2);
    zhat2(fi,:)=yhat2(fi,:)*H'+epsobs2(fi,:);
    %
    uu1=randn(1,N);uu2=randn(1,N);
    e1=ystar3(fi,:)<=tar;
    e2=ystar3(fi,:)>tar;
    yhat3(fi,:) = ((xhat3*bb1 + uu1*csigma1)*e1)+...
        ((xhat3*bb2 + uu2*csigma2)*e2);
    zhat3(fi,:)=yhat3(fi,:)*H'+epsobs3(fi,:);
    end
