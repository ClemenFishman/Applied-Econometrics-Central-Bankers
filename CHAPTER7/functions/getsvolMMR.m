function[hnew,naccept]= ...
    getsvolMMR(fbig,Qbig,mubig,betaf,hlast,mu0,is0,y,x,rho)
NS=cols(is0);
T=rows(y);
naccept=zeros(T+1,1);
hnew=zeros(T+1,NS);
...
[hh,nc]=svolttCSSNOTRENDMR(hnew(tt-1,:),hleadx,holdx,Fmat,Qmat,iQmat,cQQ,varcoef,y(tt-1,:),x(tt-1,:),mumat,rho);
...
[hh,nc]=svoltCSSNOTRENDMR(hnew(tt-1,:),holdx,Fmat,Qmat,iQmat,cQQ,varcoef,y(tt-1,:),x(tt-1,:),mumat,rho);
